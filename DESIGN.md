...
| | 说明 |
|---|---|
| **解决什么** | 每个支持 BYOK 的 App 都要重写一遍 provider 配置页、Key 输入、引导文案、Keychain 存储、连接测试。AnyLLM 把这层封装成可复用组件。 |
...
### Goals
- 拖进来即用：`AnyLLMSettingsView()` 一行出一个完整配置页。
...
```
AnyLLM (package)
├── AnyLLMCore        // 纯 Swift：数据模型 + 内置 provider 注册表 + onboarding 元数据。零依赖、零 UI。
├── AnyLLMStore       // Keychain 凭证存储 + 配置持久化。依赖 AnyLLMCore。
├── AnyLLMClient      // LLM 调用抽象协议 + 「测试连接」。依赖 AnyLLMCore。
│   └── AnyLLMClientAnyLanguageModel  // 默认适配器（独立 target，可不引入）
├── AnyLLMUI          // SwiftUI 组件。依赖 Core / Store / Client。
└── AnyLLM            // 伞 target：re-export 全部，提供一行式 API。
```

依赖原则：**Core 不依赖任何第三方包**。只有引入 `AnyLLMClientAnyLanguageModel` 时才会拉进 AnyLanguageModel。想用别的 SDK 的人完全不碰它。
...
let package = Package(
    ...
    products: [
        ...
        .library(name: "AnyLLMClientAnyLanguageModel", targets: ["AnyLLMClientAnyLanguageModel"]),
    ],
    dependencies: [
        .package(url: "https://github.com/mattt/AnyLanguageModel", from: "0.x.0"),
    ],
    targets: [
        .target(name: "AnyLLMCore"),
        .target(name: "AnyLLMStore", dependencies: ["AnyLLMCore"]),
        .target(name: "AnyLLMClient", dependencies: ["AnyLLMCore"]),
        .target(name: "AnyLLMClientAnyLanguageModel",
                dependencies: ["AnyLLMClient",
                               .product(name: "AnyLanguageModel", package: "AnyLanguageModel")]),
        .target(name: "AnyLLMUI", dependencies: ["AnyLLMCore", "AnyLLMStore", "AnyLLMClient"]),
        .target(name: "AnyLLM",
                dependencies: ["AnyLLMCore", "AnyLLMStore", "AnyLLMClient", "AnyLLMUI"]),
        .testTarget(name: "AnyLLMTests", dependencies: ["AnyLLM"]),
    ]
)
```
...
struct SettingsScreen: View {
    var body: some View {
        AnyLLMSettingsView()                 // 完整配置中心：已配列表 + 添加 + 编辑
            .anyLLMClient(AnyLanguageModelClient())   // 注入调用层
            .anyLLMProviders(.builtin)                // 或自定义子集
    }
}
```
...
| 组件 | 作用 |
|---|---|
| `AnyLLMSettingsView` | 配置中心：已添加的 configuration 列表、启用开关、+ 添加 |
...
```swift
AnyLLMSettingsView()
    .anyLLMProviders([.openAI, .anthropic, .ollama])     // 限定厂商
    .anyLLMTheme(.init(accent: .brand, cornerRadius: 12)) // 跟随宿主 App 风格
...
// 设置页
AnyLLMSettingsView().environmentObject(store)
...
| **M2 UI** | ProviderPicker / ConfigForm / KeyField / ConnectionTest / OnboardingGuide + `AnyLLMSettingsView`。 |
