...
enum AnyLLMPreview {
    ...
    static func store() -> ConfigurationStore {
        let store = ConfigurationStore(
            ...
            activeIDDefaults: UserDefaults(suiteName: "anyllm.preview")!
        )
...
#Preview("Settings") {
    AnyLLMSettingsView()
        .environmentObject(AnyLLMPreview.store())
        .anyLLMProviders(AnyLLMPreview.providers)
        .anyLLMClient(AnyLLMPreview.Client())
}

#Preview("Provider Picker") {
    NavigationStack {
        ProviderPickerView(providers: AnyLLMPreview.providers) { _ in }
    }
}

#Preview("Config Form") {
    NavigationStack {
        ProviderConfigForm(provider: AnyLLMPreview.openAI) { _, _ in }
    }
    .anyLLMClient(AnyLLMPreview.Client())
}

#Preview("Onboarding Guide") {
    OnboardingGuideView(provider: AnyLLMPreview.openAI)
}

#Preview("Badges") {
    HStack(spacing: 12) {
        ProviderBadge(provider: AnyLLMPreview.openAI, size: 48)
        ProviderBadge(provider: AnyLLMPreview.ollama, size: 48)
    }
    .padding()
}
#endif
